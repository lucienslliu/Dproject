...
{
    this->textElements.Append(TextElement(Thread::GetMyThreadId(), text, color, pos));
}
...
DebugTextRenderer::DrawText3D(const String& text, const float4& color, const point& pos)
{
    ...
    Math::float4 screenPosResName = float4::transform(pos, viewProj);
    screenPosResName.x() /= screenPosResName.w();
    screenPosResName.y() /= screenPosResName.w();

    screenPosResName.x() = (screenPosResName.x() + 1.0f) * 0.5f;
    screenPosResName.y() = 1.0f - ((screenPosResName.y() + 1.0f) * 0.5f);

    this->DrawText(text,  color, Math::float2(screenPosResName.x(), screenPosResName.y()));
}
