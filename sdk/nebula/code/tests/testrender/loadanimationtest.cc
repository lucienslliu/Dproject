...
LoadAnimationTest::Run()
{
    if (this->SetupRuntime())
    {
        ...
        this->Verify(!this->resServer->HasSharedResource(animName));

        // test synchronous loading
        Ptr<AnimResource> anim0 = this->resServer->CreateSharedResource(animName, AnimResource::RTTI, StreamAnimationLoader::RTTI).downcast<AnimResource>();
        ...
        this->resServer->UnregisterSharedResource(anim0.upcast<Resource>());
        anim0 = 0;
        this->Verify(!this->resServer->HasSharedResource(animName));

        // test asynchronous resource loading
        anim0 = this->resServer->CreateSharedResource(animName, AnimResource::RTTI, StreamAnimationLoader::RTTI).downcast<AnimResource>();
        n_assert(anim0.isvalid());
        this->Verify(this->resServer->HasSharedResource(animName));
