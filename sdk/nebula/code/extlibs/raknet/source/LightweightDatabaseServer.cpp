...
void LightweightDatabaseServer::OnUpdateRow(RakPeerInterface *peer, Packet *packet)
{
	...
	if (databaseTable==0)
	{
		RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow databaseTable==0\n");
		return;
	}
	if (databaseTable->allowRemoteUpdate==false)
	{
		RAKNET_DEBUG_PRINTF("Warning: LightweightDatabaseServer::OnUpdateRow databaseTable->allowRemoteUpdate==false\n");
		return;
	}
	...
	for (i=0; i < numCellUpdates; i++)
	{
		...
		{
			RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow cellUpdates deserialize failed i=%i numCellUpdates=%i\n",i,numCellUpdates);
			return;
		}
	}
	...
	if ((RowUpdateMode)updateMode==RUM_UPDATE_EXISTING_ROW)
	{
		if (hasRowId==false)
		{
			...
			if (row==0)
				RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow updateMode==RUM_UPDATE_EXISTING_ROW hasRowId==false");
		}
		else
		{
			...
			{
				if (row==0)
					RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow row = databaseTable->table.GetRowByID(rowId); row==0\n");
				else
					RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow row = databaseTable->table.GetRowByID(rowId); databaseTable->onlyUpdateOwnRows && RowHasIP\n");
	...
	else if ((RowUpdateMode)updateMode==RUM_UPDATE_OR_ADD_ROW)
	{
		...
		if (row==0)
		{
			...
			if (row==0)
			{
				RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow updateMode==RUM_UPDATE_OR_ADD_ROW; row=AddRow; row==0\n");
				return;
			}
		}
		else
		{
			...
			if (databaseTable->onlyUpdateOwnRows && RowHasIP(row, packet->systemAddress, databaseTable->SystemAddressColumnIndex)==false)
			{
				...
				RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow updateMode==RUM_UPDATE_OR_ADD_ROW; databaseTable->onlyUpdateOwnRows && RowHasIP. packet->systemAddress=%s sysAddr=%s\n",
	...
	else
	{
		...
		if (row==0)
		{
			RAKNET_DEBUG_PRINTF("ERROR: LightweightDatabaseServer::OnUpdateRow updateMode==RUM_ADD_NEW_ROW; row==0\n");
			return;
		}
	}
