...
struct FileListReceiver
{
	...
	bool isCompressed;
	int  filesReceived;
	DataStructures::Map<unsigned int, MemoryBlock> pushedFiles;
...
FileListReceiver::FileListReceiver() {filesReceived=0; partLength=1; DataStructures::Map<unsigned int, MemoryBlock>::IMPLEMENT_DEFAULT_COMPARISON();}
...
void FileListTransfer::Send(FileList *fileList, RakPeerInterface *rakPeer, SystemAddress recipient, unsigned short setID, PacketPriority priority, char orderingChannel, bool compressData, IncrementalReadInterface *_incrementalReadInterface, unsigned int _chunkSize)
{
	...
	if (anythingToWrite)
	{
		...
		for (i=0; i < fileList->fileList.Size(); i++)
		{
			...
			RakAssert(fileList->fileList[i].dataLengthBytes!=0);
			if (fileList->fileList[i].dataLengthBytes==0)
			{
				RakAssert(fileList->fileList[i].dataLengthBytes!=0);
				continue;
			}
...
void FileListTransfer::Clear(void)
{
	...
	{
		delete [] filesToPush[i].fileListNode.filename;
	}
	filesToPush.Clear();
}
